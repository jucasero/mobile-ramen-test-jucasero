...
const Header: React.FC<IProps> = (props) => {
  ...
  return <IonHeader className="ion-no-border">
    ...
    <div className="title">
        ...
        {props.user?.unique_name && <div>{`${props.user?.unique_name}`} <EmojiIcon emoji="👋🏼" size="medium" /></div>}
    </div>
    <div className="lava-buttons">
      <IonButton className={props.activeTasks ? 'white active' : 'white'} onClick={() => onSlideClickHandler(0)}>
        {props.activeTasks ? <TaskIconActive /> : <TaskIcon />}
        {localize('TASKS', '')}
      </IonButton>
      <IonButton className={props.activeTools ? 'white active' : 'white'} onClick={() => onSlideClickHandler(2)}>
