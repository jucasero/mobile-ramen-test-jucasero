...
import { IonModal, IonContent, IonFooter, IonButton, IonHeader, IonToolbar, IonButtons, IonIcon, useIonAlert } from '@ionic/react';
import { App } from '@capacitor/app';
import { AuthenticationClient, i18, IJwtEntity, Expr, XConsole } from "@team_eureka/eureka-ionic-core";
...
const UserMenu: React.FC<IProps> = ({
  ...
  const getAppInfo = async () => {
    Expr.whenInNativePhone(async () => {
      ...
      setVersion(`${packageJSON.version} - base ${appInfo.version}-${appInfo.build}`)
    })
  }

  return (
    <Fragment>
      <IonModal swipeToClose={false} backdropDismiss={false} className="user-full-menu" isOpen={isOpen}>
        <IonHeader className="ion-no-border box-and-icon">
          <IonToolbar>
            <IonButtons slot="start" >
              {!onClose ? null :
                <IonButton className="white" onClick={() => {onClose()}} >
        ...
        <IonFooter>
          <XButton background='black' size='large' onClick={onSignOutClickHandler}>
            {localize('SIGN_OUT', '')}
          </XButton>
          <div >
